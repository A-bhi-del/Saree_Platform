...
import { getfollowers } from "../api/friendsSystem";
import { useTheme } from "../context/ThemeContext";
import FollowButton from "../components/FollowButton";
// import FollowButton from "./FollowButton"; // 👈 Integrated Reusable FollowButton
...
function Followers() {
  ...
  if (loading) {
    return (
      ...
      >
        <p className="animate-pulse text-sm font-medium">Loading followers...</p>
      </div>
    );
  }
        ...
        {followersList.length === 0 ? (
          ...
          <ul className="space-y-4">
            {followersList.map((user) => {
              const { _id, name, email, profileImage, isFollowing } = user;
                ...
                >
                  ...
                  <FollowButton
                    ...
                    onStatusChange={(newStatus) => {
                      // Optional: Parent state me follow status track karne ke liye
                      setFollowersList((prev) =>
                        prev.map((u) =>
                          u._id === _id ? { ...u, isFollowing: newStatus } : u
                        )
                      );
                    }}
                  />
